...
//		pos_inicial = roda.position()*3.141592/180*raio;
//		t_inicial = Time::now();
//		usleep(1000*delay);
//
//		for(int i = 0; i < 6 ; i++){
...
//			velo_instantanea = (pos_final - pos_inicial)/delta_t.count(); // count retorna em seg
//			velo_final_med += velo_instantanea;
//
...
void Controlador_motor::loop_controlador(){
	...
	for(int i = 0; i < 5; i++){
		...
		t_final = Time::now();
		delta_t = t_final-t_inicial;
		vetor_velo[i] = (pos_final - pos_inicial)/delta_t.count(); // count retorna em seg
		tempo_total += delta_t.count();
	...
	while(!thread_finalizar){
		for(int i = 0; i< 4 ; i++)
			vetor_velo[i] = vetor_velo[i+1];

		pos_final = roda.position()*3.141592/180*raio;
		t_final = Time::now();
		delta_t = t_final-t_inicial;
		vetor_velo[4] = roda.speed();//(pos_final - pos_inicial)/delta_t.count(); // count retorna em seg
		tempo_total += delta_t.count();

		for(int i = 0; i< 5 ; i++)
			velo_final_med += vetor_velo[i];
